classdef LaguerrePolynomial < Polynomial
    % LAGUERREPOLYNOMIAL Construct a LaguerrePolynomial.
    % SYS=LAGUERREPOLYNOMIAL(DEG) constructs polynomial system returned in
    % SYS, representing an orthogonal Laguerre polynomial of order DEG.
    % Example (<a href="matlab:run_example LaguerrePolynomial">run</a>)
    % sys=LaguerrePolynomial(3);
    % See also
    ...
    properties
        % The parameter DEG of the LaguerrePolynomial(DEG) system. DEG is
        % the order of the orthogonal Laguerre polynomial system.
        deg
    end
    
    methods
        function sys=LaguerrePolynomial(deg)
            % LAGUERREPOLYNOMIAL Construct a LegendrePolynomial.
            % SYS=LAGUERREPOLYNOMIAL(DEG) constructs polynomial system 
