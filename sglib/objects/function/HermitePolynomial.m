classdef HermitePolynomial < Polynomial
    % HERMITEPOLYNOMIAL Construct a HermitePolynomial.
    % SYS=HERMITEPOLYNOMIAL(DEG) constructs polynomial system returned in
    % SYS, representing an orthogonal Hermite polynomial of order DEG.
    % Example (<a href="matlab:run_example HermitePolynomial">run</a>)
    % sys=HermitePolynomial(3);
    %
    % See also LEGENDREPOLYNOMIAL POLYNOMIAL 
    ...
    properties
        % The parameter DEG of the HermitePolynomial(DEG) system. DEG is
        % the order of the orthogonal Hermite polynomial system.
        deg
    end
    
    methods
        function sys=HermitePolynomial(deg)
            % HERMITEPOLYNOMIAL Construct a HermitePolynomial.
            % SYS=HERMITEPOLYNOMIAL(DEG) constructs polynomial system
